...
export async function POST(req: NextRequest) {
  ...
  if (!title || !category_id || !price) return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL!;
  const service = process.env.SUPABASE_SERVICE_ROLE_KEY!;
  const supabase = createClient(url, service);
  const { data: prod, error: e1 } = await supabase.from('products').insert({ title, description, category_id, price, mrp, in_stock, tags }).select('id').single();
