...
export async function POST(req: NextRequest) {
  ...
  // ✅ NOW including quantity from body
  const {
    title,
    description,
    category_id,
  ...
  // ✅ Make sure we always insert some quantity (0 if not provided)
  const safeQuantity =
    typeof quantity === 'number' && !Number.isNaN(quantity)
      ? quantity
      : 0;

  // 1) Insert product WITH image_url and quantity
  const { data: prod, error: e1 } = await supabase
    ...
    .insert({
      ...
      tags,
      image_url: primary,
      quantity: safeQuantity, // 👈 NEW: save quantity
    })
    .select('id')
    .single();
